...
public class Improved {
	...
	public static ArrayList PSA(ArrayList doublepoint, int counter, int shifttimes, int windowsize) {
		...
		int cutpoint = 0;
		int mincutpoint = 1000000000;
		double cutline = 0;
		double finalcutline = 0;

		if (distancex >= distancey) {
			
			for (int cutnum = 1; cutnum < shifttimes; cutnum++) {

				for (int i = 0; i < doublepoint.size(); i++) {
					
					cutline = distancex*cutnum/shifttimes + MinX(doublepoint);
					Point eachpoint = (Point) doublepoint.get(i);
					if ((eachpoint.getX() >= cutline - distancex/windowsize)&&(eachpoint.getX() <= cutline + distancex/windowsize)){
						cutpoint = cutpoint + 1;
					}				
				}
			...
			for (int i = 0; i < doublepoint.size(); i++) {
				...
				Point eachpoint = (Point) doublepoint.get(i);
				if ((eachpoint.getX()) <= finalcutline) {
			...
			for (int cutnum = 1; cutnum < shifttimes; cutnum++) {
				...
				for (int i = 0; i < doublepoint.size(); i++) {
					...
					cutline = distancey*cutnum/shifttimes + MinY(doublepoint);
					Point eachpoint = (Point) doublepoint.get(i);
					if ((eachpoint.getY() >= cutline - distancey/windowsize)&&(eachpoint.getY() <= cutline + distancey/windowsize)){
						cutpoint = cutpoint + 1;
					}				
				}
			...
			for (int i = 0; i < doublepoint.size(); i++) {
				...
				Point eachpoint = (Point) doublepoint.get(i);
				if ((eachpoint.getY()) <= finalcutline) {
					eachpoint.setId(eachpoint.getId());
					clusterA.add(eachpoint);
				} else {
					eachpoint.setId(clusternum + 1);
		...
		PSA(newlist, counter,shifttimes,windowsize);
	...
	public static void main(String[] args) throws IOException {
		...
		String Improvedresult = "/Users/Jia/Desktop/dataset/r15_Improvedresult.txt";      /////////
		int clusternum = 15;                                                          /////////
		int shifttimes = 10;                                                           /////////
		int windowsize = 20;                                                          /////////
        //////////////////////////////////////////////////////////////////////////////////////
		...
		ArrayList result = PSA(stringtopoint(input), clusternum,shifttimes,windowsize);

		writeFile(Improvedresult, result, input);
		long endTime=System.currentTimeMillis();
		System.out.println("*****Improved PSA Clustering Result*****");
		System.out.println();
		
	    ExternalEvaluation ee=new ExternalEvaluation();
	    ee.main(Improvedresult,origin,clusternum);
	    //System.out.println();
	    InternalEvaluation ie=new InternalEvaluation();
	    ie.main(Improvedresult,clusternum);
